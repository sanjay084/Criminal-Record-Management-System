...
<div class="card ">
  ...
  <div style="background-image:url('admin/images/<?=$_REQUEST['back']?>');height:350px;width:550px">
  <img src="img/<?=$_REQUEST['image']?>" style="width:200px;padding-top:50px;margin-left:30px;height:340px">
  <img src="img/<?=$_REQUEST['image']?>" style="width:250px;padding-top:10px;margin-left:30px;height:250px">
  </div>
...
<input type="hidden" name="image"  value="<?=$_REQUEST['image']?>" class="form-control">
<input type="hidden" name="id" value="<?=$_REQUEST['id']?>" class="form-control">
price-
<?php
$result=select("select price from items where elecid='".$_REQUEST['id']."'");
while($r=mysqli_fetch_array($result))
{extract($r);

?>
<input type="text" readonly value="<?=$price?>"  class="form-control"></br>
 ...
 if(isset($_REQUEST['book']))
	  {
		  ...
		  extract($_REQUEST);
		 $n=iud("INSERT INTO `cart`(`elecid`, `username`, `image`,`address`) VALUES ('$id','$name','$image','$address')");
		  if($n==1)
		  {
